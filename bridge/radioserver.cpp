...
bool serviceBridge(int fd) {
  ...
  switch (kind) {
    ...
    default:
      // An unknown kind means the stream has desynchronised, and carrying on
      // would be reading one message as another.
      fprintf(stderr, "radioserver: unknown engine message 0x%02x\n", kind);
      return false;
  }
  return true;
}
